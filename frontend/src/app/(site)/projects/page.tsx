...
export default async function ProjectsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const projects = await publicApi.projects({ page: params.page ?? 1, category: params.category, technology: params.technology }).catch(() => ({ items: [], meta: { total: 0, page: 1, limit: 10, totalPages: 1, hasNextPage: false, hasPrevPage: false } }));
  return (
    <>
