...
export default async function BlogPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [posts, categories, tags] = await Promise.all([
    publicApi.posts({ page: params.page ?? 1, categorySlug: params.category, tagSlug: params.tag }).catch(() => ({ items: [], meta: { total: 0, page: 1, limit: 10, totalPages: 1, hasNextPage: false, hasPrevPage: false } })),
