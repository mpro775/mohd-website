...
import { datetimeLocalToUtc } from "../utils/post-date";
  ...
  const action = async (name: string) => {
    ...
    try {
      ...
      const body = name === "schedule"
        ? { expectedVersion: post.version, scheduledAt: datetimeLocalToUtc(scheduleValue), sourceTimezone: Intl.DateTimeFormat().resolvedOptions().timeZone }
        : { expectedVersion: post.version };
      const result = await clientApiRequest<Post>(`/blog/posts/${post.id ?? post._id}/${name}`, { method: "POST", body });
      setPost(result.data); form.reset(valuesFromPost(result.data)); toast.success(result.message);
