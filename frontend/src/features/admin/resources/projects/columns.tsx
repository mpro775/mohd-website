...
export const projectColumnLabels: Record<string, string> = {
  title: "اسم المشروع",
  category: "التصنيف",
  status: "الحالة",
  isPublished: "النشر",
...
export function createProjectColumns({
  ...
  return [
        ...
        return (
          <div className="h-10 w-16 rounded border border-border overflow-hidden bg-muted/40 flex items-center justify-center select-none shrink-0">
            {imageUrl ? (
              ...
              <span className="text-[10px] text-muted-foreground">بلا غلاف</span>
            )}
          </div>
        );
      },
    ...
    // 4. Category
    {
      ...
      header: "التصنيف",
      cell: ({ row }) => (
        <span className="inline-flex items-center rounded-full bg-secondary/15 px-2.5 py-0.5 text-xs font-bold text-secondary uppercase">
