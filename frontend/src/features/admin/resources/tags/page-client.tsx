...
export function TagsPageClient() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: TagFormValues) => {
      if (editingTag) {
        ...
        return adminClient.updateResource<Tag>("blog/tags", id, values);
      } else {
        return adminClient.createResource<Tag>("blog/tags", values);
      }
    },
    onSuccess: (res: any) => {
  ...
  const actionMutation = useMutation({
    mutationFn: async ({ id, action }: { id: string; action: "activate" | "deactivate" }) => {
      return adminClient.updateResource<Tag>(`blog/tags`, `${id}/${action}`, {});
    },
