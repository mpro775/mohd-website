...
export const postsData = [
  {
    title: "كيف أبني منصة SaaS قابلة للتوسع؟ الدروس التي تعلمتها من بناء منتجات حقيقية",
    slug: "how-to-build-scalable-saas-platform",
    summary: "دليل عملي لبناء منصات SaaS قابلة للتوسع اعتماداً على تجارب حقيقية في تطوير المنتجات الرقمية.",
    excerpt: "أخطاء شائعة ودروس مهمة في بناء منصات SaaS متعددة العملاء وإدارة النمو التقني للمنتج.",
    ...
    seo: {
      ...
      metaDescription: "دليل شامل لبناء منصات SaaS احترافية متعددة العملاء وقابلة للنمو."
    },
    categorySlug: "saas",
    tagSlugs: ['typescript', 'nestjs', 'react', 'docker', 'saas', 'multi-tenant', 'scalability', 'software-architecture'],
  ...
  {
    title: "الذكاء الاصطناعي ليس بديلاً للمطورين: كيف أستخدم AI لبناء منتجات أفضل",
    slug: "ai-for-building-better-products",
    summary: "رؤية عملية حول استخدام الذكاء الاصطناعي في تطوير المنتجات الرقمية بدلاً من اعتباره بديلاً للمبرمجين.",
    excerpt: "كيف يمكن للذكاء الاصطناعي أن يضاعف إنتاجية المطورين والشركات بدلاً من استبدالهم.",
    status: PostStatus.PUBLISHED,
    readTime: 12,
    isFeatured: true,
    allowIndexing: true,
    seo: {
      ...
      metaDescription: "تجربة عملية في دمج الذكاء الاصطناعي داخل المنتجات الرقمية ومنصات SaaS."
    },
    categorySlug: "artificial-intelligence",
    tagSlugs: ['typescript', 'nestjs', 'react', 'docker', 'artificial-intelligence', 'openai', 'qdrant', 'n8n', 'automation'],
  ...
  },
  {
    ...
    summary: "خريطة طريق عملية لتحويل الأفكار إلى منتجات رقمية حقيقية قابلة للنمو.",
    ...
    seo: {
      ...
      metaDescription: "دليل عملي لتحويل الأفكار إلى منتجات رقمية قابلة للنمو والاستثمار."
    },
    categorySlug: "product-development",
    tagSlugs: ['startup', 'mvp', 'product-management', 'software-engineering', 'clean-code', 'performance', 'scalability'],
...
export async function seedPosts(
  ...
  if (!author) {
    throw new Error(`Admin user not found with email: ${adminEmail}. Run seed first.`);
  }

  for (const data of postsData) {
    // Resolve category
    const categoryDoc = await categoryModel.findOne({ slug: data.categorySlug });
    if (!categoryDoc) {
      console.warn(`Category with slug "${data.categorySlug}" not found! Skipping post.`);
      continue;
    }
    ...
    if (existing) {
      ...
      await postModel.updateOne({ slug: data.slug }, { $set: postPayload } as any);
    } else {
      console.log(`Creating post: ${data.title}`);
      await postModel.create(postPayload as any);
    }
  }
